...
export type CalendarDeleteInput = { activityId: string };
...
export function parseCreateInput(body: unknown): CalendarCreateInput {
  ...
  if (!isRecord(body.behavior)) {
    throw new HttpError(400, "behavior is required");
  }
...
function coerceChangeValue(path: string, value: unknown): unknown {
  if (DATE_PATHS.has(path)) {
    return parseOptionalDate(value, path);
  }
  ...
  if (path === "behavior.flexibility") {
    return parseFlexibility(value);
  }
  if (path === "behavior.recurrence.rule") {
  ...
  if (path === "behavior.recurrence.days") {
    ...
    if (!days) {
      throw new HttpError(400, "behavior.recurrence.days is invalid");
    }
    return days;
  }
  if (path === "reminders") {
    ...
    if (!reminders) {
      throw new HttpError(400, "reminders is invalid");
    }
    return reminders;
  }
  if (path === "schedule.timezone") {
    if (typeof value !== "string") {
      throw new HttpError(400, "schedule.timezone must be a string");
    }
...
export function parseUpdateInput(body: unknown, idFromRoute?: string): CalendarUpdateInput {
  ...
  if (!isRecord(body.changes)) {
    throw new HttpError(400, "changes is required");
  }
  ...
  if (Object.keys(changes).length === 0) {
    throw new HttpError(400, "changes cannot be empty");
  }
...
function parseBound(value: unknown, timezone: string, edge: "start" | "end"): Date {
  if (typeof value !== "string") {
    throw new HttpError(400, `${edge} bound is invalid`);
  }
...
export function parseSuggestInput(
  body: unknown,
  fallbackTimezone: string,
