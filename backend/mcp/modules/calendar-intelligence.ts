...
// Note: calendar.split_task already exists in calendar module, so we don't duplicate it here
...
export function registerCalendarIntelligenceModule(registry: ToolRegistry): void {
  registry.registerTool(preferencesSaveTool);
  registry.registerTool(preferencesGetTool);
  registry.registerTool(preferencesUpdateTool);
  registry.registerTool(preferencesDeleteTool);
  registry.registerTool(capacityCheckTool);
  registry.registerTool(missedReviewTool);
  registry.registerTool(previewTool);
